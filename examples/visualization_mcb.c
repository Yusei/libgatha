...
boolean add_to_chart(GathaMcbData *mcbdata, int *actions, payoff_t *payoffs,
		     void* data)
{
  ...
  gatha_pvect_timeline_add(d->pvect_timeline, mcbdata->proba[0]);
  gatha_pvect_timeline_add(d->pvect_timeline2, mcbdata->proba[1]);

  gatha_report_add_payoffs(d->report, payoffs);
  gatha_report_add_pvect(d->report, mcbdata->proba);

  if (mcbdata->time % 10 == 0) printf("%d\n", mcbdata->time);
...
void init_probas_randomly(GathaGame* g, proba_t **p)
{
  ...
  for(i=0 ; i<n ; i++) {
    c = 0;
    for(j=0 ; j<m ; j++) {
      p[i][j] = (int) ((((double)rand())/RAND_MAX) * 20) + 1;
    }
  }
